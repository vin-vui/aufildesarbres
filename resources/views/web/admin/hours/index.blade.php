                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center">
                                            <div class="flex-shrink-0 h-12 w-12 bg-cover bg-center rounded-full" style="background-image: url({{ url('storage/'.$hour->image) }})"></div>
                                        </div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $hour->title }}</div>
                                    </td>

                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $hour->description }}</div>
                                    </td>

                                    <td class="pl-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="edit({{ $hour->id }})" class="rounded p-1">
                                    ...
                                    <td class="pr-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if($confirming === $hour->id)
                                        <button wire:click="delete({{ $hour->id }})" class="bg-red-500 rounded p-1">
                                            <svg class="h-6 w-6 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </button>
                                        @else
                                        <button wire:click="confirmDelete({{ $hour->id }})" class="rounded p-1">
